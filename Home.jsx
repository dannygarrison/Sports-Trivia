...
const GAMES = [
  ...
  {
    id: "who-has-more",
    path: "/games/who-has-more",
    title: "Who Has More?",
    sport: "NFL",
    sports: ["NFL", "NBA"],
    description: "Pick the player with the higher career stat. NFL and NBA legends go head-to-head.",
    tag: "VERSUS",
    available: true,
    dateAdded: 12,
    plays: 0,
  ...
  {
    id: "draft-class-quiz",
    path: "/games/draft-class-quiz",
    title: "Draft Class Quiz",
    sport: "NFL",
    description: "Pick a draft year and name every first-round pick before time runs out.",
    tag: "FREE RECALL",
    available: true,
    dateAdded: 13,
    plays: 0,
  ...
  {
    id: "who-wore-it",
    path: "/games/who-wore-it",
    title: "Who Wore It?",
    sport: "NFL",
    sports: ["NFL", "NBA"],
    description: "Name the famous players who wore iconic jersey numbers across the NFL and NBA.",
    tag: "JERSEY TRIVIA",
    available: true,
    dateAdded: 14,
    plays: 0,
  ...
  {
    id: "nfl-career-path",
    path: "/games/nfl-career-path",
    title: "NFL Career Path",
    sport: "NFL",
    description: "Guess the NFL player from their career journey. Teams revealed one at a time — fewer clues = more points.",
    tag: "GUESS WHO",
    available: true,
    dateAdded: 16,
    plays: 0,
  ...
  {
    id: "nba-career-path",
    path: "/games/nba-career-path",
    title: "NBA Career Path",
    sport: "NBA",
    description: "Guess the NBA player from their career journey. Teams revealed one at a time — fewer clues = more points.",
    tag: "GUESS WHO",
    available: true,
    dateAdded: 17,
    plays: 0,
    ...
    <div style={{
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom: 16 }}>
        <div style={{ display: "flex", gap: 6 }}>
          {(game.sports || [game.sport]).map(s => (
            <span key={s} style={{
              fontSize: 9, fontWeight: 800, fontFamily: "'Oswald', sans-serif",
              letterSpacing: 3, textTransform: "uppercase",
              color: SPORT_META[s].accent, background: SPORT_META[s].accent + "18", padding: "3px 8px", borderRadius: 4,
            }}>{s}</span>
          ))}
        </div>
        <span style={{
...
export default function Home() {
  ...
  const filteredGames = sortGames(GAMES.filter(g => {
    const matchesSport = activeTab === "ALL" || g.sport === activeTab || (g.sports && g.sports.includes(activeTab))
    const q = search.toLowerCase()
    const sportAliasMatch = Object.entries(SPORT_ALIASES).some(([sport, aliases]) =>
      (g.sport === sport || (g.sports && g.sports.includes(sport))) && aliases.some(alias => alias.includes(q) || q.includes(alias))
    )
