...
export default function DraftClassQuiz() {
  ...
  const timeUp = timeLeft <= 0 && gameActive;

  useEffect(() => {
    if (gameActive && timeLeft > 0 && !finished) {
      timerRef.current = setTimeout(() => setTimeLeft(t => t - 1), 1000);
      return () => clearTimeout(timerRef.current);
    }
    if (timeLeft <= 0 && gameActive) setGameActive(false);
  }, [gameActive, timeLeft, finished]);

  const startGame = (year) => { setSelectedYear(year); setFound(new Set()); setInput(""); setTimeLeft(TIMER_SECONDS); setGameActive(true); setGaveUp(false); trackPlay(); setTimeout(() => inputRef.current?.focus(), 100); };
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#07070f", backgroundImage: "radial-gradient(ellipse at 50% 0%, #0d0d1f 0%, #07070f 60%)", color: "#f0f0f0", fontFamily: "'Oswald', sans-serif", padding: "84px 16px 60px" }}>
        <Helmet>
          ...
          <meta name="description" content="Can you name every first-round NFL Draft pick? Pick a year from the 2020s and race the clock. Test your draft knowledge at TrivialSports." />
          <meta property="og:title" content="Draft Class Quiz – Name Every First-Round Pick – TrivialSports" />
          <meta property="og:description" content="Can you name every first-round NFL Draft pick? Pick a year and race the clock." />
        ...
        <div style={{ maxWidth: 600, margin: "0 auto", animation: "fadeUp 0.4s ease both" }}>
          ...
          <p style={{ fontSize: 13, color: "#c8a050", margin: "0 0 28px", fontFamily: "Georgia, serif" }}>Pick a draft year. You have {TIMER_SECONDS / 60} minutes to name every first-round pick.</p>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#07070f", backgroundImage: "radial-gradient(ellipse at 50% 0%, #0d0d1f 0%, #07070f 60%)", color: "#f0f0f0", fontFamily: "'Oswald', sans-serif", padding: "84px 16px 60px" }}>
      <Helmet>
        ...
        <meta name="description" content={`Can you name all ${total} first-round picks from the ${selectedYear} NFL Draft? Race the clock at TrivialSports.`} />
      ...
      <div style={{ maxWidth: 700, margin: "0 auto 24px", animation: "fadeUp 0.4s ease both" }}>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: "#c8a050", margin: "7px 0 0", fontFamily: "Georgia, serif" }}>Name all {total} first-round picks.</p>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: 9, letterSpacing: 3, textTransform: "uppercase", color: "#ffffff25", marginBottom: 2 }}>Time</div>
              <div style={{ fontSize: 26, fontWeight: 900, fontVariantNumeric: "tabular-nums", color: timeLeft <= 15 && gameActive ? "#e74c3c" : finished ? "#c8a050" : "#ffffff" }}>{formatTime(timeLeft)}</div>
            </div>
            <div style={{ textAlign: "center" }}>
      ...
      {(finished || timeUp || gaveUp) && (
        <div style={{ maxWidth: 700, margin: "0 auto 20px", background: gaveUp || timeUp ? "linear-gradient(135deg,#1a0808,#2a1010)" : "linear-gradient(135deg,#0f0d00,#1a1500)", border: `1px solid ${gaveUp || timeUp ? "#e74c3c33" : "#c8a05044"}`, borderRadius: 14, padding: "16px 24px", textAlign: "center", animation: "popIn 0.4s ease both" }}>
          <div style={{ fontSize: 22, fontWeight: 900, color: gaveUp || timeUp ? "#e74c3c" : "#c8a050", letterSpacing: 1, textTransform: "uppercase" }}>
            {finished ? `All ${total} picks named!` : gaveUp ? `Gave up – ${found.size} of ${total} found` : `Time's up – ${found.size} of ${total} found`}
          </div>
        </div>
      )}
      ...
      <div style={{ maxWidth: 700, margin: "0 auto", border: "1px solid #ffffff0a", borderRadius: 12, overflow: "hidden", animation: "fadeUp 0.5s ease both", animationDelay: "0.1s" }}>
        {picks.map((p, i) => {
          const isFound = found.has(p.pick); const reveal = (gaveUp || timeUp) && !isFound; const isFlashing = flash === p.pick;
          return (
            <div key={p.pick} style={{ display: "grid", gridTemplateColumns: "52px 1fr auto", gap: 0, borderBottom: i < picks.length - 1 ? "1px solid #ffffff07" : "none", background: isFlashing ? "#c8a05015" : isFound ? "#0a120008" : i % 2 === 0 ? "#09090f" : "#07070d", transition: "background 0.3s" }}>
              ...
              <div style={{ padding: "10px 12px", display: "flex", alignItems: "center", fontSize: 10, color: "#ffffff15", fontFamily: "'Oswald', sans-serif", letterSpacing: 1 }}>
                {(isFound || reveal) && <span style={{ color: isFound ? "#ffffff25" : "#e74c3c25" }}>{p.team}</span>}
              </div>
            </div>
          );
        })}
